...
export function LiveFeed({ events }: { events: HydraEvent[] }) {
  ...
  const filtered = useMemo(
    () => filter === 'llm' ? events.filter((e) => LLM_DRIVEN_EVENT_TYPES.has(e.type)) : events,
    [filter, events],
  );
      ...
      <CardHeader>
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-1">
            ...
            <FilterPill active={filter === 'llm'} onClick={() => setFilter('llm')}>LLM only</FilterPill>
          </div>
        </div>
        <span className="text-[10px] text-subtle font-mono">{filtered.length} of {events.length}</span>
      </CardHeader>
          ...
          <AnimatePresence initial={false}>
            {filtered.length === 0 ? (
              <li className="px-5 py-12 text-center text-xs text-subtle">
                {filter === 'llm' ? 'No LLM decisions yet…' : 'Waiting for events…'}
              </li>
            ) : (
