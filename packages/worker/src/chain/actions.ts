...
export function encodeIncreaseLiquidity(p: {
  tokenId: bigint;
  liquidity: bigint;
  amount0Max: bigint;
  amount1Max: bigint;
...
export function encodeDecreaseLiquidity(p: {
  tokenId: bigint;
  liquidity: bigint;
  amount0Min: bigint;
  amount1Min: bigint;
...
export function encodeBurnPosition(p: {
  tokenId: bigint;
  amount0Min: bigint;
  amount1Min: bigint;
  hookData?: Hex;
...
export function encodeTakePair(
  currency0: Address,
  currency1: Address,
  recipient: Address,
): Hex {
...
export function encodeUnlockData(actionCodes: number[], params: Hex[]): Hex {
  ...
  const actionsHex = concatHex(
    actionCodes.map((c) => `0x${c.toString(16).padStart(2, "0")}` as Hex),
  );
  return encodeAbiParameters(
