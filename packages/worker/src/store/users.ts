...
export type UserRow = {
  doId: string;
  wallet: string;       // OWNER wallet (PK-derived)
  signerWallet: string; // wallet that signed in via Reown
  tokenId: string;
  registeredAt: number;
  lastKick: number;
};
...
export async function upsertUser(
  db: D1Database,
  u: { doId: string; wallet: string; signerWallet: string; tokenId: string },
): Promise<void> {
  await db
    .prepare(
      `INSERT INTO users (do_id, wallet, signer_wallet, token_id, registered_at, last_kick)
       VALUES (?, ?, ?, ?, ?, 0)
    ...
    .bind(u.doId, u.wallet.toLowerCase(), u.signerWallet.toLowerCase(), u.tokenId, Date.now())
    .run();
}
...
export async function listAllUsers(db: D1Database): Promise<UserRow[]> {
  const r = await db
    .prepare(
      `SELECT do_id, wallet, signer_wallet, token_id, registered_at, last_kick FROM users ORDER BY registered_at DESC LIMIT 200`,
    )
    .all<{ do_id: string; wallet: string; signer_wallet: string; token_id: string; registered_at: number; last_kick: number }>();
  return (r.results ?? []).map((x) => ({
    doId: x.do_id,
